...
### macOS
- macOS (Apple Silicon)
- Command Line Tools・Determinate Nix（下記セットアップの手順 1・2 でインストール）
...
`Command Line Tools をインストール → Determinate Nix をインストール → git で clone → nix-darwin をブートストラップ → gh で SSH 鍵を登録 → remote を SSH に切替` の順で進めます。
...
Homebrew（cask のインストール）が Command Line Tools を必須要件として要求するため、最初に入れておきます。これで本物の `git` も入るので、この後の `git clone` が `/usr/bin/git` のスタブに邪魔されず普通に通ります（フル Xcode は不要、CLT だけで十分）。
...
手順 1 で入れた Command Line Tools の `git` を使います。
...
> **Note:** ブートストラップ後は `home/common/git.nix` により nix 管理の `git` が PATH 優先になります。

#### 4. nix-darwin をブートストラップ
