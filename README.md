...
### macOS
- macOS (Apple Silicon)
- Determinate Nix（下記セットアップの手順 1 でインストール）
...
`Determinate Nix をインストール → nix 提供の git で clone → nix-darwin をブートストラップ → gh で SSH 鍵を登録 → remote を SSH に切替` の順で進めます。

#### 1. Determinate Nix をインストール

macOS では [Determinate Nix の macOS パッケージ](https://dtr.mn/determinate-nix)（`.pkg`）を使います。ダウンロードした `.pkg` をダブルクリックしてインストールしてください。

> **Note:** `curl ... | sh` の CLI インストーラでも入りますが、`/nix` 用の暗号化 APFS ボリュームの作成・マウント段で失敗することがあります（`Volume on diskNsM failed to mount`）。`.pkg` 版は macOS のインストーラ機構に任せるぶんこの段に強いため推奨します。CLI 版で失敗して中断した場合は、残った `Nix Store` ボリュームを `diskutil apfs list` で確認して `sudo diskutil apfs deleteVolume <識別子>` で削除し、**再起動してから** `.pkg` を実行してください。

#### 2. nix 提供の git でリポジトリを clone

macOS の `/usr/bin/git` は Command Line Tools を要求するスタブなので、clone は nix 提供の git で行います。インストールを促すダイアログが出たら **「キャンセル」** で閉じてください（Command Line Tools は不要です）。

```sh
nix run nixpkgs#git -- clone https://github.com/YutaUra/nix-darwin.git ~/.config/nix-darwin
cd ~/.config/nix-darwin
```

> **Note:** 次の手順 3 のブートストラップ後は `home/common/git.nix` により git が PATH に入るため、以降は普通に `git` が使えます。

#### 3. nix-darwin をブートストラップ
...
> **Note:** `Unexpected files in /etc, aborting activation` が出て `/etc/nix/nix.custom.conf` が挙げられた場合、Determinate Nix の `.pkg` が置いたデフォルトファイルと、このリポジトリが管理する同名ファイル（`common/default.nix` の `environment.etc."nix/nix.custom.conf"`）が衝突しています。エラーの指示どおり退避してから再実行してください。中身はこの後 nix-darwin 管理の symlink に置き換わります。
>
> ```sh
...
> ```
>
> 同様に `/etc/nix/nix.conf` や `/etc/shells` で競合した場合も、バックアップして退避するかエラーメッセージの指示に従ってください。

#### 4. gh で GitHub に SSH 鍵を登録
